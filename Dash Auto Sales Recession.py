...
app.layout = html.Div([
    ...
    html.H1("Automobile Sales Statistics Dashboard",style={'textAlign':'center','color':'#503D36', 'fontsize':40}),#May include style for title
    html.Div([#TASK 2.2: Add two dropdown menus
        html.Label("Select Statistics:"),
          dcc.Dropdown(id='dropdown-statistics', 
                  ...
                  style={'width':80%, 'height':'50px', 'fontsize':20. 'text-align-last':'center'})
    ]),
...
#Plot 2 Calculate the average number of vehicles sold by vehicle type       
        ...
        R_chart2  = dcc.Graph(
            figure=px.bar(average_sales,
                x='Year'
                y='Sales by vehicle type'
                title='Average Vehicles Sold by Vehicle Type')

# Plot 3 Pie chart for total expenditure share by vehicle type during recessions
        # use groupby to create relevant data for plotting
        exp_rec=recession_data.groupby('Vehicle type')['Automobile_Sales'].sum().reset_index()
        names= exp_rec.index
...
# Plot 4 bar chart for the effect of unemployment rate on vehicle type and sales
        ...
        R_chart4=dcc.Graph(
            figure=px.bar(unemp_rate,
                x='Year'
                y='Vehicles Sales'
                title='Vehicles sold by vehicle type and Unemployment Rate'
...
#TASK 2.6: Returning the graphs for displaying Recession data
         return [
 ## Note: Value for the children parameter can be R_chart1, R_chart2 so on or Y_chart1,Y_chart2 so on depending on the type of charts.
...
#plot 1 Yearly Automobile sales using line chart for the whole period.
        ...
        Y_chart1 = dcc.Graph(
            figure=px.line(yas, 
                x='Year',
                y='Average Automobile Sales',
                title="Average Automobile Sales fluctuation over Non-Recession Period")

        ))

# Plot 2 Total Monthly Automobile sales using line chart.
       y_average_sales =yearly_data.groupby('Year', 'Vehicle_Type')[Automobile_Sales].mean().reset_index()                           
        Y_chart2  = dcc.Graph(
            figure=px.bar(y_average_sales,
                x='Year'
                y='Sales by vehicle type'
                title='Average Vehicles Sold by Vehicle Type')
            # Plot bar chart for average number of vehicles sold during the given year
# Plot 3
        y_avr_data=yearly_data.groupby('Year')['Automobile_Sales'].mean().reset_index()
        Y_chart3 = dcc.Graph( 
            figure=px.bar(y_avr_data,
            title='Average Vehicles Sold by Vehicle Type in the year {}'.format(input_year)))

# Total Advertisement Expenditure for each vehicle using pie chart
        exp_data=yearly_data.groupby('Advertising_Expenditure')['Automobile_Sales'].sum().reset_index()
